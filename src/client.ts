...
export class RealtimeClient {
  private config: Required<T.RealtimeConfig>;
  ...
  constructor(config: T.RealtimeConfig) {
    this.config = {
      authMode: "header",
      reconnect: true,
  ...
  async connect() {
    ...
    try { await this.createTransport(); } 
    catch (e) { this._state = "disconnected"; throw e; }
  }
  ...
  private async createTransport() {
    ...
    this.transport.onMessage = (data) => {
      ...
      try {
        ...
        if (this.config.dedup && this.dedup.has(msg.message_id)) return;
        if (this.config.dedup) this.dedup.add(msg.message_id);
    ...
    this.transport.onClose = () => {
      ...
      this.reconnect.attempt(
        async (n) => { this.emitter.emit("reconnecting", n); await this.createTransport(); this.emitter.emit("reconnected"); },
        () => { this._state = "disconnected"; this.emitter.emit("reconnect_failed"); }
      );
    };
